...
def update_order_view(request,pk):
    ...
    if request.method=='POST':
        ...
        if orderForm.is_valid():
            ...
            if updated_order.status == 'Out for Delivery':
                # Update the quantity of the product
                product = updated_order.product
                product.quantity += updated_order.quantity                    
                product.save()
                messages.info(request, str(order.price) + ' taka has been refunded successfully!')
...
def cart_view(request):
    ...
    if request.method == "POST":
        customer = models.Customer.objects.get(user=request.user)  # Fetch the customer based on logged-in user
        purchased_products = []
...
def customer_address_view(request):
    ...
    if product_ids_cookie:
        product_ids = [int(id) for id in product_ids_cookie.split('|')]
        products_in_cart = models.Product.objects.filter(id__in=product_ids)
        
        # Calculate the total price of the order and update product quantities
        total_price = 0
        ...
        # Create an order record in your database and process the payment
        # ...
...
def return_product(request, order_id, product_id):
    if request.method == 'POST':
        # Retrieve the quantity to return from the form
        return_quantity = int(request.POST.get('quantity', 0))
        ...
        main_quantity=order.quantity-math.floor(order.quantity/product.required_quantity)*product.free_quantity
        rem_quantity=main_quantity-return_quantity

        main_free_quantity=math.floor(rem_quantity/product.required_quantity)*product.free_quantity
        tot_rem_quantity=main_free_quantity+rem_quantity
...
def admin_ret(request):
    return_requests = models.ReturnRequest.objects.all()
    return render(request, 'ecom/admin_return_request.html', {'return_requests': return_requests})
